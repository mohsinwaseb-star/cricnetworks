...
function RankMovement({ rank, prev }) {
  ...
  if (diff < 0) return <ChevronDown size={12} className="text-[#A8E63D]" />;
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
        ...
        <Link to="/rankings" className="text-xs text-[#A8E63D] hover:underline font-medium">
          Full Rankings
        </Link>
      </div>
        ...
        {data.map((item) => (
          <div key={item.rank} className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition-colors cursor-pointer">
            <div className="w-5 flex items-center justify-center">
              <span className={`text-sm font-bold ${item.rank <= 3 ? 'text-[#A8E63D]' : 'text-gray-500'}`}>
                {item.rank}
              </span>
            </div>
